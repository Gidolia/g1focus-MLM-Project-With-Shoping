...
$rtey=mysql_query("SELECT * FROM `distributor` WHERE `d_id`=".$_POST[d_id]);
	...
	{ $tiy=mysql_fetch_assoc($rtey);
        ...
        if($plus>0)
        {
        mysql_query("INSERT INTO `transition_history` (`1`, `2`, `5`, `10`, `20`, `50`, `100`, `500`, `1000`, `amount`, `a_bal`, `d_id`, `date`, `method`, `type`, `b_bal`, `branch_id`, `e_id`, `slip_no`, `slip_place`, `time`) VALUES ('$_POST[m1]', '$_POST[m2]', '$_POST[m5]', '$_POST[m10]', '$_POST[m20]', '$_POST[m50]', '$_POST[m100]', '$_POST[m500]', '$_POST[m1000]', '$_POST[bal]', '$plus', '$_POST[d_id]', '$o_date', '$_POST[method]', '+', '$tiy[bal]', '$_SESSION[shop_id]', '$_SESSION[e_id]', '$_POST[slip]', '$_SESSION[shop_id]', '$time')")or die("transtation not recorded");
