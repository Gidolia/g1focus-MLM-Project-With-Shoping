...
if($rr)
  ...
  $rd_id=$rd_id+1;
			...
			mysql_query("INSERT INTO `rd` (`rd_id`, `d_id`, `start_date`, `end_date`, `percentage`, `month`, `days`, `clear_amount`, `bal`, `installment_period`, `installment_qty`, `clear_date`, `received`, `per_installment`, 	`rd_slip_no.`, `e_id`, `rd_status`, `shop_id`) VALUES ('$rd_id', '$_POST[d_id]', '$o_date', '$datee', '$_POST[percent]', '$_POST[month]', '', '$rty', '$_POST[amount]', 'monthly', '$_POST[month]', '$date', '1', '$_POST[amount]', '$_POST[formno]', '$_SESSION[e_id]', 'a', '$_SESSION[shop_id]')")or die("Rd info not selected");
			
			mysql_query("INSERT INTO `rd_info` (`rd_id`, `date`, `due_days`, `late_install_charge`, `charge_percantage`, `amount`, `d_id`, `e_id`) VALUES ('$rd_id', '$o_date', '0', '0', '2', '$_POST[amount]', '$_POST[d_id]', '$_SESSION[e_id]')")or die("not inserted");
			
		
			mysql_query("INSERT INTO `transition_history` (`10`, `20`, `50`, `100`, `500`, `1000`, `amount`, `a_bal`, `d_id`, `date`, `method`, `type`, `b_bal`, `branch_id`, `e_id`, `to`, `time`) VALUES ('', '', '', '', '', '', '$_POST[amount]', '$pluse', '$_POST[d_id]', '$o_date', 'transfer RD', '-', '$tiy[bal]', '$_SESSION[shop_id]', '$_SESSION[e_id]', 'RD', '$time')")or die("transtation not recorded");
...
$dd='Your%20a/c%20'.$_POST[d_id].'%20Your%20RD%20Started%20sucessfully%20RD%20no.%20'.$rd_id.'.%20%201st%20installment%20received%20Rs%20'.$_POST[amount].'%20Your%20RD%20Bal%20Rs%20'.$_POST[amount].'%20%20%20Your%20BV%20bal%20'.$o_bv.'%20Thankyou';
$url = 'http://sms.hspsms.com/sendSMS?username=cmd&message='.$dd.'&sendername=gonefs&smstype=TRANS&numbers='.$tiy[mobile].'&apikey=e8ab1258-683f-45cf-ab3e-082efac6a9b3';
