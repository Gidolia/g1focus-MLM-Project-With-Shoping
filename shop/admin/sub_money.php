...
$rtey=mysql_query("SELECT * FROM `distributor` WHERE `d_id`=".$_POST[d_id]);
	...
	{ $tiy=mysql_fetch_assoc($rtey);
        ...
        if($plus>0)
        {
        mysql_query("INSERT INTO `transition_history` (`10`, `20`, `50`, `100`, `500`, `1000`, `amount`, `a_bal`, `d_id`, `date`, `method`, `type`, `b_bal`, `branch_id`, `e_id`, `chequeno`, `slip_no`, `time`) VALUES ('$_POST[m10]', '$_POST[m20]', '$_POST[m50]', '$_POST[m100]', '$_POST[m500]', '$_POST[m1000]', '$_POST[bal]', '$plus', '$_POST[d_id]', '$o_date', '$_POST[method]', '-', '$tiy[bal]', 'admin', '$_SESSION[e_id]', '$_POST[cheno]', '$_POST[slip_no]', '$time')")or die("transtation not recorded");
