...
//$d_idtt=200001;
//Empty both Table
function find_under_bv($d_idtt,$date)
{
$temp=array();
$temp1=array();
$grand_total=0;
...
{
	$temp[]=$fet1[d_id];
	$grand_total=$grand_total+find_bv($fet1[d_id],$date);
...
for($x=0;$x<count($temp);$x++)
{
	...
	while($fet3=mysql_fetch_assoc($sel3))
	{
		...
		$grand_total=$grand_total+find_bv($fet3[d_id],$date);
	}
}
unset($temp);
$temp=array();
...
for($x=0;$x<count($temp1);$x++)
{
	...
	while($fet5=mysql_fetch_assoc($sel5))
	{
		...
		$grand_total=$grand_total+find_bv($fet5[d_id],$date);
	}
}
unset($temp1);
$temp1=array();
...
{   
	...
	while($fet13=mysql_fetch_assoc($sel13))
	{
		...
		$grand_total=$grand_total+find_bv($fet13[d_id],$date);
	}
	}
unset($temp);
$temp=array();
	 ...
	 ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	  for($x=0;$x<count($temp1);$x++)
	  {
		  ...
		  while($fet15=mysql_fetch_assoc($sel5))
		  {
			  ...
			  $grand_total=$grand_total+find_bv($fet15[d_id],$date);
		  }
	  }
unset($temp1);
$temp1=array();
...
//function 
function find_bv($did,$date)
{
	$b=0;
	$ii=0;
	$w = "select sum(bv) as total_bv from bill where d_id='$did' and status='c' and o_date='$date'";
...
{
	if($pert>=50 and $pert<=149){$avv=12;}
	elseif($pert>=150 and $pert<=299){$avv=13;}
	elseif($pert>=300 and $pert<=624){$avv=16;}
	elseif($pert>=625 and $pert<=1249){$avv=20;}
	elseif($pert>=1250 and $pert<=2499){$avv=23;}
	elseif($pert>=2500 and $pert<=4999){$avv=27;}
	elseif($pert>=5000 and $pert<=9999){$avv=31;}
	...
	elseif($pert>=115000 and $pert<=169999){$avv=54;}
	elseif($pert>=170000 and $pert<=259999){$avv=56;}
	elseif($pert>=260000 and $pert<=349999){$avv=58;}
	elseif($pert>=350000){$avv=60;}
